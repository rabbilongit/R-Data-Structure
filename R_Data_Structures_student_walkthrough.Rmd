...
```{r}
student_grades <- c(94,96,43,08,75,82)
student_name <- c('Jhon','Jake', 'Jimmy', 'Jane','Jason','Jenny')
student_count <- length(student_name)

student_list <- list(student_grades,student_name,student_count)
...
```{r}
student_name <- c('Jhon','Jake',"jenny")
student_grade_level <-  c(
  'Sophomore','Jniour','Senior'
)
...
```{r}
stdeunt_grades <- df$student_class_grade
stdeunt_grades
```
...
my_matrix <- matrix(
  ...
  ncol <- 5,
  byrow <-FALSE,
  dimnames <- list(c('row1','row2','row3'),c('col1','col2','col3','col4', 'col5' ))
)
...
my_array <-  array(
  ...
  dim <-c(3,3,2),
  #row/column names
  dimnames <- list(c("row1",'row2','row3'), c('col1','col2','col3'),c('Mat1','mat2'))
)
...
my_factor <- factor(c('blue','green','brown','grey'))
my_factor
```
...
```{r}
my_factor <- factor(c('blue','green','brown','grey'),
                  levels = c('green','grey','brown','blue')
                    )
my_factor
```
...
```{r}
first_matrix <-  matrix(first_vetor <- c(1,2,3,4,5,6,7,8,9),nrwo <- 3)
second_matrix <- matrix(second_vecto <- c(9,8,7,6,5,4,3,2,1),nrow <- 3)
first_matrix
second_matrix
```
